  ...
  if($NUM_OF_PASSENGERS<=0 || $PRICE<=0){
    $NOTIFICATION="<p>Enter a positive number</p>";
      ...
      if(strtolower($SOURCE)==strtolower($DESTINATION)){
        ...
        $sql="UPDATE `ticket` SET `BUS_ID`='$BUS_ID',`CUSTOMER_ID`='$CUST_ID',`SOURCE`='$SOURCE',`DESTINATION`='$DESTINATION',`PRICE`='$PRICE',`NUM_OF_PASSENGERS`='$NUM_OF_PASSENGERS' WHERE `ticket`.`TICKET_ID`='$ID'";
        $result=mysqli_query($con,$sql);
        if($result){
  ...
  <div class='container my-5'>
    <form action='' method='post'>
      ...
      <div class='dropdown1'>
        ...
        while($row=mysqli_fetch_assoc($result)){
        $CUST_ID=$row['CUST_ID'];
        echo'<option name="CUSTOMER_ID" value='.$CUST_ID.'>'.$CUST_ID.'</option>';
        }
        }
        ?>
          </select>
      ...
      <div class='mb-3'>
        ...
        <input type='text' class='form-control' id='SOURCE' name='SOURCE' placeholder='Enter the source of passenger' autocomplete='off' required value="<?php
          echo $SOURCE;
          ?>"
          >
      </div>
